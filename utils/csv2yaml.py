...
with open(infile, "r") as csvfile:
    ...
    for row in csvreader:
        if not sound_file and row["sound_file"]:
            output.append(
                yaml_head.format(sound_file=row["sound_file"], uid=gen_uid(infile))
            )
        ...
        output.append(
            yaml_vocab.format(
                kana=row["kana"], english=row["english"], kanji=row["kanji"]
            )
        )
...
if (
    ...
    or input("overwrite existing %s ?(y/n)" % outpath) == "y"
):
