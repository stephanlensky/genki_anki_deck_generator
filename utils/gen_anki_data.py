...
def apply_hacks(cat, i, tag, stag=None):
    ...
    if i == 0 and tag == "Others":
        cat["vocabulary"].insert(
            6, {"japanese": "〜ばん", "english": "number ...", "kanji": ""}
        )
    if i == 0 and tag == "Expressions":
        del cat["vocabulary"][len(cat["vocabulary"]) - 1]
        cat["vocabulary"].append(
            {"japanese": "そうですか。", "english": "I see.", "kanji": ""}
        )
        cat["vocabulary"].append(
            {"japanese": "そうですか？", "english": "Is that so?", "kanji": ""}
        )
...
def get_subtags(vocab, tag):
    tags = [
        tuple(
            remove_from_list(
                list(filter(lambda x: not x.endswith("Vocabulary"), x["tags"])), tag
            )
        )
        for x in vocab
    ]
...
for i, deck in enumerate(vocab):
    ...
    if i == 0:
        tag_vocab = list(
            filter(
                lambda x: any([y.endswith("Vocabulary") for y in x["tags"]])
    ...
    #        , deck))
        ...
        tag_vocab = list(
            filter(
                lambda x: any([f"L{i + 1}_Vocabulary" == y for y in x["tags"]]),
                all_past_cards,
            )
        )
    if i == 1:
        ...
        tags = [
            tuple(filter(lambda e: not e.endswith("Vocabulary"), x["tags"]))
            for x in tag_vocab
        ]
    ...
    for t in tags:
        ...
        if i in [0, 1] or len(subtags) in [0, 1]:
            cat["vocabulary"] = [
                {"japanese": v["jap"], "english": v["eng"], "kanji": v["kan"]}
                for v in vocs
            ]
...
vocab_1 = vocab[0]
tag_vocab = list(
    filter(lambda x: any([y == "L1_Greetings" for y in x["tags"]]), vocab_1)
)
tags = [
    tuple(filter(lambda e: not e.endswith("Vocabulary"), x["tags"])) for x in tag_vocab
]
tags = unique(tags)
tags = list(filter(lambda x: len(x) == 1, tags))
...
for t in tags:
    ...
    subtags = get_subtags(vocs, tag)
    cat["vocabulary"] = [
        {"japanese": v["jap"], "english": v["eng"], "kanji": num2kanji(v["eng"])}
        for v in vocs
    ]
    for v in cat["vocabulary"]:
        if "／" in v["japanese"]:
...
tag_vocab = list(
    filter(
        lambda x: any([y.endswith("Additional_Vocabulary") for y in x["tags"]]), vocab_1
    )
)
tags = [
    tuple(filter(lambda e: not e.endswith("Vocabulary"), x["tags"])) for x in tag_vocab
...
for t in tags:
    ...
    if len(subtags) in [0, 1]:
        cat["vocabulary"] = [
            {"japanese": v["jap"], "english": v["eng"], "kanji": v["kan"]} for v in vocs
        ]
        apply_hacks(cat, i, tag)
    else:
        scats = []
...
for t in tags:
    ...
    cat["vocabulary"].insert(
        0, {"japanese": "ひゃく", "english": "100", "kanji": num2kanji("100")}
    )
    for v in cat["vocabulary"]:
        if (
            "っ" in v["japanese"]
...
vocab_1 = vocab[6]
tag_vocab = list(
    filter(lambda x: any([y == "Useful_Expressions" for y in x["tags"]]), vocab_1)
)
tags = [tuple(["Useful_Expressions"])]
...
for t in tags:
    ...
    subtags = get_subtags(vocs, tag)
    cat["vocabulary"] = [
        {"japanese": v["jap"], "english": v["eng"], "kanjis": v["kan"]} for v in vocs
    ]
    apply_hacks(cat, i, tag)
    out_dict["cards"].append(cat)
